...
function Env(name) {
    ...
    const notify = (t, s, m) => { if (isSurge || isLoon || isEgern) $notification.post(t, s, m); if (isQX) $notify(t, s, m) };
    const msg = (t, s, m) => { if (isSurge || isLoon || isEgern) $notification.post(t, s, m); if (isQX) $notify(t, s, m); console.log(`${t}\n${s}\n${m}`) };
    const log = console.log;
    const done = v => { $done(v) };
    return { name, isLoon, isSurge, isQX, isEgern, http, setdata, getdata, setval, getval, notify, msg, log, done };
}
